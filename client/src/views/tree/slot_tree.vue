      ...
      <span slot-scope="{ node, data }" class="slot-t-node">
        ...
        <span v-show="!node.isEdit">
          ...
          <span class="slot-t-icons">
            <!-- 新增按钮 -->
            <el-button icon="el-icon-plus" circle :size="iconSize" type="success" @click="NodeAdd(node, data)" />
            <!-- 编辑按钮 -->
            <el-button icon="el-icon-edit" circle :size="iconSize" type="primary" @click="NodeEdit(node, data)" />
            <!-- 删除按钮 -->
            <el-button icon="el-icon-delete" circle :size="iconSize" type="warning" @click="NodeDel(node, data)" />
          </span>
        </span>
        <!-- 编辑输入框 -->
        <span v-show="node.isEdit">
...
<script>
import api from '@/utils/shengshi'
export default {
  name: 'SlotTree',
  data() {
    return {
