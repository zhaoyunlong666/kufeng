  ...
  <div>
    ...
    <el-card>
      ...
      <el-table key="userlist" :data="userlist" border stripe>
        ...
        <el-table-column prop="out_trade_no" label="商家单号" />
        <el-table-column prop="out_trade_no" label="系统单号" />
        <el-table-column prop="address" label="客户手机" />
        <el-table-column prop="Usertype_id" label="订单名" />
        <el-table-column prop="nickname" label="单价" />
        <el-table-column prop="reg_time" label="Pay状态" />
        <el-table-column prop="phone" label="支付方式" />
        <el-table-column prop="phone" label="交易银行" />
        <el-table-column prop="reg_time" label="下单时间" />
        <el-table-column prop="reg_time" label="回调时间" />
        <!-- <el-table-column prop="kecheng_type" label="关注公众号" /> -->

        <!-- <template slot-scope="scope"> |上边参数的格式 :formatter="UsertypeFormat"
...
export default {
  ...
  methods: {
    ...
    async search() {
      // 搜索事件
      const { data: ress } = await this.$http.get('/api/order/list', {
    ...
    // 动态获取用户数据
    async getuserlist() {
      const { data: res } = await this.$http.get('/api/order/list', {
        params: this.queryInfo
      })
