    ...
    <el-card>
      ...
      <el-row :gutter="20">
        <el-col :span="7">
          ...
          <el-input ref="phone" v-model="phone" type="text" placeholder="服务名称" autocomplate="off" @input="changeInput" @change="getuserlist" @keyup.enter.native="getuserlist">
            <el-button slot="append" icon="el-icon-search" @click="getuserlist" />
          </el-input>

        </el-col>
        <el-col :span="4">
          <el-button type="primary" @click="handaddnews()">服务名称</el-button>
           <el-dialog
              title="添加服务"
      ...
      <el-table key="userlist" :data="userlist" border stripe>
        ...
        <el-table-column prop="nickname" label="部门名称" />
        ...
        <el-table-column label="操作">
          <template slot-scope="scope">
            <!--添加子子部门-->
             <el-tooltip effect="dark" content="添加该子部门" placement="top" :enterable="false">
              <el-button type="primary" icon="el-icon-edit" size="mini" @click="handleEdit2(scope.$index,scope.row)" />
            </el-tooltip>
...
export default {
  ...
  methods: {
    ...
    async search() {
      // 搜索事件
      const { data: ress } = await this.$http.get('/api/bumen/list', {
    ...
    // 动态获取用户数据
    async getuserlist() {
      const { data: res } = await this.$http.get('/api/bumen/list', {
        params: this.queryInfo
      })
