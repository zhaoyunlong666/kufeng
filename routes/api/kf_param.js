...
const kf_param = require("../../models/kf_param");
...
router.post("/register", (req, res) => {
  //查询用户是否存在
  kf_param.findOne({where:{ users: req.body.users }}).then(result=> {
    if (result) {
...
router.post('/addusers',(req,res)=>{
    kf_param.findOne({whrer:{users:req.body.users}}.then(res=>{
  if(res){
...
router.post('/delusers:id',(req,res)=>{
    kf_param.destroy({where:{id:req.body.id}}).then(result =>{
    if (result){
...
router.post('/updateusers:id',(req,res)=>{
    kf_param.findOne({where:{id:req.body.id}}).then(users=>{
    if(res){
...
router.post('/login',(req, res, next) => {
  let md5 = crypto.createHash("md5");
  let password = md5.update(req.query.password).digest("hex"); //.digest("hex")对接收的参数进行加密
  kf_param.findOne({ where: { u_username: req.query.username, u_password: password } }).then(kf_param => {
    if (kf_param !== null) {
     
      //3是管理身份其他身份不可以登录
      if (password === kf_param.u_password) {
        Cookies.set('token', req.query.usersname)
        Cookies.set('sidebarStatus',0)
        //{"code":20000,"data":{"token":"admin-token"}}
        res.send({ code: 200, msg: '管理登录成功', data: tokens.admin })
      } else {
      ...
      res.send({ code: 400, msg: '账号或者密码错误' })
    }
  })
  // console.log(res.query.usersname)
...
console.log('进入')
  ...
  await kf_param.findAndCountAll({
...
router.post("/edit:id", (req, res) => {
  kf_user.findOne({where:{id}}).then(kf_user => {
    if(kf_user){
