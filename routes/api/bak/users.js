...
router.post("/register", (req, res) => {
  //查询用户是否存在
  User.findOne({where:{ users: req.body.users }}).then(result=> {
    if (result) {
...
router.post('/login',(req, res, next) => {
  ...
  User.findOne({ where: { username: req.query.username, password: password } }).then(User => {
    if (User !== null) {
      //3是管理身份其他身份不可以登录
      if (password === User.password && User.userstype_id === 3) {
        ...
        res.send({ code: 401, msg: '目前不支持用户登录' })
      }
    } else {
...
router.get('/list', async (req, res, next) => {
  ...
  let { pagenum, pagesize, nickname, phone } = req.query
