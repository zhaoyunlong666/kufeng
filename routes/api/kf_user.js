...
const kf_user = require("../../models/kf_user");
...
router.post("/register", (req, res) => {
  //查询用户是否存在
  kf_user.findOne({where:{ users: req.body.users }}).then(result=> {
    if (result) {
...
router.post('/addusers',(req,res)=>{
    user.findOne({whrer:{users:req.body.users}}.then(res=>{
  if(res){
...
router.post('/updateusers:id',(req,res)=>{
    user.findOne({where:{id:req.body.id}}).then(users=>{
    if(res){
...
router.post('/login',(req, res, next) => {
  ...
  kf_user.findOne({ where: { u_username: req.query.username, u_password: password } }).then(kf_user => {
    if (kf_user !== null) {
     
      //3是管理身份其他身份不可以登录
      if (password === kf_user.u_password) {
